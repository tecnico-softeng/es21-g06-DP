...
public class QuestionsXmlImport {
    ...
    private QuestionDetailsDto importOpenAnswerQuestion(Element questionElement) {
        String correctAnswer = questionElement.getChild("correct_answer").getValue().trim();
        OpenAnswerQuestionDto openAnswerQuestionDto = new OpenAnswerQuestionDto();
        openAnswerQuestionDto.setCorrectAnswer(correctAnswer);
    ...
    private QuestionDetailsDto importMultipleChoiceQuestion(Element questionElement) {
        ...
        String choiceType = questionElement.getAttributeValue("choiceType");
        if (choiceType != null) {
            multipleChoiceQuestionDto.setChoiceType(ChoiceType.valueOf(choiceType));
        }
        ...
        for (Element optionElement : questionElement.getChild("options").getChildren("option")) {
            ...
            if (choiceType != null && ChoiceType.valueOf(choiceType) == ChoiceType.SORTING && !order.equals("null")) {
                Integer optionOrder = Integer.valueOf(order);
    ...
    private QuestionDetailsDto importItemCombinationQuestion(Element questionElement) {
        ...
        var groups = new ArrayList<ItemGroupDto>();
        for (Element groupElement : questionElement.getChild("itemGroups").getChildren("itemgroup")) {
            var group = new ItemGroupDto();
            group.setSequence(Integer.valueOf(groupElement.getAttributeValue("sequence")));
            var items = new ArrayList<ItemDto>();
            for (Element itemElement : groupElement.getChildren("item")) {
                ...
                HashSet<Integer> itemCombinations = new HashSet<>();
                String combinations = itemElement.getAttributeValue("combinations");
                if(combinations.length() > 0) {
                    itemCombinations = new HashSet<>(
                        Arrays.stream(combinations.split(",")).map(Integer::parseInt).collect(Collectors.toSet()));
                }
        ...
        questionDto.setItemGroups(groups);
        return questionDto;
    }
