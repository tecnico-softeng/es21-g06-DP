...
public class XMLQuestionExportVisitor implements Visitor {
    ...
    public void visitQuestionDetails(MultipleChoiceQuestion question) {
        ...
        if (ChoiceType.SINGLE_SELECTION != question.getChoiceType()) {
            this.currentElement.setAttribute("choiceType", question.getChoiceType().toString());
        }
    ...
    public void visitQuestionDetails(ItemCombinationQuestion question) {
        ...
        Element groupsElement = new Element("itemGroups");
        this.currentElement.addContent(groupsElement);

        this.currentElement = groupsElement;
        question.visitItemGroups(this);
    }

    @Override
    public void visitItemGroup(ItemGroup group) {
        Element groupElement = new Element("itemgroup");

        groupElement.setAttribute("sequence", String.valueOf(group.getSequence()));
        this.currentElement.addContent(groupElement);
    ...
    public void visitItem(Item item) {
        ...
        for (Integer i : item.getCombinations()) {
            builder.append(i);
            builder.append(",");
        }
        if(builder.length() > 0)
            builder.deleteCharAt(builder.length()-1);
        String itemCombinations = builder.toString();
        
        itemElement.setAttribute("combinations", itemCombinations);
    ...
    public void visitOption(Option option) {
        ...
        optionElement.setAttribute("content", option.getContent());
        optionElement.setAttribute("correct", String.valueOf(option.isCorrect()));
        if (option.getQuestionDetails().getChoiceType() == ChoiceType.SORTING) {
