...
Cypress.Commands.add('cleanItemCombinationQuestionsByName', questionName => {
  dbCommand(`WITH toDelete AS (SELECT qt.id as question_id FROM questions qt JOIN question_details qd ON qd.question_id = qt.id and qd.question_type='item_combination' where title like '%${questionName}%')
                  , groupToDelete AS (SELECT id FROM  item_group WHERE question_details_id IN (SELECT qd.id FROM toDelete JOIN question_details qd on qd.question_id = toDelete.question_id))
                  , itemToDelete AS (SELECT id FROM items WHERE item_combination_id IN (SELECT id FROM groupToDelete))
                  , comb AS (DELETE FROM combination WHERE OWNER_ID IN (SELECT id FROM itemToDelete))
                  , item AS (DELETE FROM items WHERE item_combination_id IN (SELECT id FROM groupToDelete))
                  , grp AS (DELETE FROM item_group WHERE id IN (SELECT id FROM groupToDelete))
                  , det AS (DELETE FROM question_details WHERE question_id in (SELECT question_id FROM toDelete))
                DELETE FROM questions WHERE id IN (SELECT question_id FROM toDelete);`);
});